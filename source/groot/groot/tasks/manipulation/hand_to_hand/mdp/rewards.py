...
def rew_palm_alignment(env: ManagerBasedRLEnv,
    ...
    """
    Encourage palms to face each other when both hands are near the object.

    - Palm position: hand_pitch_link origin with -Z offset (same convention as grasp flags).
    - Palm normal: -Z axis of each hand in world frame.
    - Score: average of (left normal · dir_to_right) and (right normal · dir_to_left).
      Positive when palms face each other, negative when knuckles face each other.
    """
    l_palm, r_palm, n_l, n_r = _palm_pos_and_dir(env, palm_offset)

    # direction from left to right palm
    dir_lr = r_palm - l_palm
    dir_lr = dir_lr / torch.clamp(torch.norm(dir_lr, dim=-1, keepdim=True), min=eps)

    # only shape when both hands are near the object
    left_close = torch.norm(rel_left_to_object(env), dim=-1) < near_thresh
    right_close = torch.norm(rel_right_to_object(env), dim=-1) < near_thresh
    ...
    high_enough = obj_z > 0.86
    mask = left_close & right_close & (hand_sep < hand_sep_thresh) & warm_mask & high_enough
    if not torch.any(mask):
        return torch.zeros(env.num_envs, device=env.device)

    align_l = torch.sum(n_l * dir_lr, dim=-1)      # left faces right
    align_r = torch.sum(n_r * (-dir_lr), dim=-1)   # right faces left
    score = 0.5 * (align_l + align_r)

    reward = torch.zeros(env.num_envs, device=env.device)
    reward[mask] = score[mask]
    return reward


def _palm_pos_and_dir(env: ManagerBasedRLEnv, palm_offset: float) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
...
def rew_hand_front_penalty(env: ManagerBasedRLEnv,
                           min_x: float = 0.12,
                           gate_height: float = 0.86) -> torch.Tensor:
    """
...
def rew_palm_inward(env: ManagerBasedRLEnv,
                    warmup_steps: int = 12,
                    min_height: float = 0.86,
                    thresh: float = 0.2,
                    hand_sep_max: float = 0.28,
                    palm_offset: float = 0.06) -> torch.Tensor:
    """
    ...
    score = 0.5 * (
        torch.sum(n_r * y_root_world, dim=-1) +
        torch.sum(n_l * (-y_root_world), dim=-1)
    )
...
def rew_hand_height_band_penalty(env: ManagerBasedRLEnv,
                                 min_height: float,
                                 max_height: float,
                                 weight_high: float = 1.0) -> torch.Tensor:
...
def rew_exchange_zone(env: ManagerBasedRLEnv,
                      target_offset: tuple[float, float, float] = (0.25, 0.0, 0.1),
                      sigma: float = 0.08,
                      obj_height_min: float = 0.85,
                      hand_obj_thresh: float = 0.12) -> torch.Tensor:
    """
...
def rew_close_bonus(env: ManagerBasedRLEnv,
                    hand_obj_thresh: float = 0.12,
                    hand_sep_thresh: float = 0.22,
                    min_height: float = 0.85,
                    vel_thresh: float = 0.25) -> torch.Tensor:
...
def rew_handover_bonus(env: ManagerBasedRLEnv,
                       vel_thresh: float = 0.25,
                       min_height: float = 0.8,
                       palm_align_thresh: float = 0.0) -> torch.Tensor:
    """
...
def rew_palm_sagittal(env: ManagerBasedRLEnv,
                      palm_offset: float = 0.06,
                      warmup_steps: int = 12,
                      min_height: float = 0.86,
                      hand_sep_max: float = 0.28,
                      tol: float = 0.2) -> torch.Tensor:
    """
    ...
    dev = 0.5 * (torch.abs(torch.sum(n_l * y_root_world, dim=-1)) + torch.abs(torch.sum(n_r * y_root_world, dim=-1)))
...
def rew_height_shaping_banded(env: ManagerBasedRLEnv,
                              min_height: float,
                              target_height: float,
